export type ElfHostPage = { id: string; title: string; description?: string; url: string }
export type ElfHostPageContext = {
  hostPages?: ElfHostPage[]
  allowedPageUrls?: string[]
  openHostPage?: (pageId: string, signal?: AbortSignal) => Promise<{ url: string }>
}
...
export type ElfHardRuleInput = { currentUrl: string; visible?: unknown; instruction?: string; action?: Record<string, unknown>; result?: unknown }
export type ElfHardRuleSession = {
  observe?(input: ElfHardRuleInput): Promise<{ action?: { elementId: string; method: string; arguments: unknown[]; description?: string }; stop?: boolean } | null> | { action?: { elementId: string; method: string; arguments: unknown[]; description?: string }; stop?: boolean } | null
  beforeAction(input: ElfHardRuleInput): Promise<{ allowed: boolean; reason?: string }> | { allowed: boolean; reason?: string }
  afterAction?(input: ElfHardRuleInput): Promise<void> | void
  validate(input: ElfHardRuleInput): Promise<{ success: boolean; evidence?: string }> | { success: boolean; evidence?: string }
  close?(): Promise<void> | void
}
export type ElfHardRules = {
  /** Return null explicitly when this task is outside the business rule scope. Exceptions fail closed. */
  createSession(input: { request: string; context: Record<string, unknown>; signal?: AbortSignal }): Promise<ElfHardRuleSession | null> | ElfHardRuleSession | null
}
...
export function createElf(options: ElfOptions): {
  run(text: string, context?: Record<string, unknown> & ElfHostPageContext, options?: { signal?: AbortSignal; fallbackOnSkillError?: boolean; onEvent?: (event: Record<string, unknown>) => void }): Promise<Record<string, unknown>>
  browser: ElfBrowser
  profile: ElfProfile
